...
#class FontSelector(bpy.types.PropertyGroup):
#    '''name = StringProperty() '''
#    # LISTS
#    fontselector_fontlist = bpy.props.CollectionProperty(type=FontSelectorFontList)
#    # PROPS
#    fontselector_index = bpy.props.IntProperty()
...
class FontSelectorAddFP(bpy.types.Operator):
    ...
    bl_label = ""
    bl_description = "Add Font Filepath"
                    ...
                    for file in os.listdir(path):
                        ...
                        if any(file_extension==ext for ext in extlist):
                            ...
                            for font in bpy.data.fonts:
                                if os.path.join(path, file)==os.path.abspath(bpy.path.abspath(font.filepath)):
                                    chk3=1
                                    dup=font
                            if chk3==0:
                                bpy.data.fonts.load(filepath=os.path.join(path, file))
                            elif chk3==1:
                                newf=bpy.data.fonts.load(filepath=os.path.join(path, file))
                                dup.name=newf.name
                                bpy.data.fonts.remove(newf, do_unlink=True)
    ...
    def draw(self, context):
        ...
        if len(fplist)==0:
            ...
            if fonth.fontselector_list==0:
                ...
                row=layout.row()
                row.template_list("FontUIList", "", fonth, "fontselector_list", activedata, "fontselector_index", rows=5)
